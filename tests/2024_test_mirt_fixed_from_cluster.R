...
sim_mirt_data = function(type, pars) {
  ...
  colnames(data) = c(paste0('x', 1:j), paste0('z', 1:ncol(ff)))

  # store simulated data
  sim_data = list(alpha = alpha, delta = delta, theta = theta, beta = beta)
...
save(
  output,
  file = paste0('mirt_fixed_', SLURM_ID, '.rda')
)
