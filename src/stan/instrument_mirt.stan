...
functions {
  real ordered_logistic_log_irt_vec(array[] int y, vector nu, matrix cut,
    vector eta, array[] int K, int nlong, array[] int itype, array[] real fweights) {
    real val = 0.0;
    int K_i = 0;
    int y_i = 0;
    real nu_i = 0.0;
    ...
    for(i in 1:nlong) {
      ...
      if(itype[i] < 3) {
        if (y_i == 1) {
          val += -log1p_exp(nu_i - cut[i, 1])*fweights[i];
          //val += log(inv_logit(cut[i, 1] - nu_i));
        } else if(y_i == K_i) {
          val += -log1p_exp(cut[i, K_i-1] - nu_i)*fweights[i];
          //val += log(inv_logit(nu_i - cut[i, K_i-1]));
        } else {
          val += log_inv_logit_diff(cut[i, y_i] - nu_i, cut[i, y_i-1] - nu_i)*fweights[i];
        }
      } else {
        eta_i = eta[i];
        if (y_i == 1) {
...
//array[J] int<lower=2,upper=Ncateg_max>  Ncategi;
data {
  ...
  array[J] int<lower=2,upper=Ncateg_max> Ncategi; // Number of categories for each item
  int<lower=1> N_long;              // number of long observations
  array[N_long] int<lower=1> Ncategi_jj;
  array[N_long] int<lower=1,upper=N> nn;  // participant for observation n
  array[N_long] int<lower=1,upper=J> jj;  // question for observation n
  array[N_long] int<lower=0,upper=Ncateg_max> y;   // correctness for observation n
  array[N_long] int<lower=1,upper=3> itype;      // item type for each item (1pl=1, 2pl=2, 3pl=3)
  int<lower=0,upper=1> any_eta3pl;       // any eta parameters?
  int<lower=0> nEta3pl; // number of eta parameters
  array[N_long] int<lower=0> find_eta3pl;      // find the correct eta parameter for long format of data
  ...
  array[D] int<lower=0> alpha_dstart;
  array[D] int<lower=0> alpha_dend;
  ...
  array[has_treg ? D : 0] int<lower=0> beta_dstart; // beta start index for each dimension
  array[has_treg ? D : 0] int<lower=0> beta_dend;   // beta end index for each dimension
  array[any_rand_ind ? D : 0] int<lower=0> zeta_dstart; // zeta start index for each dimension
  array[any_rand_ind ? D : 0] int<lower=0> zeta_dend;   // zeta end index for each dimension
  array[N_long] real fweights; // weights for each observation
  ...
  array[D] int<lower=0> which_dim_fixed_reg;
  array[D] int<lower=0> which_dim_fixed_reg_sort;

  array[D] int<lower=0> which_dim_ind_reg;
  array[D] int<lower=0> which_dim_ind_reg_sort;
  ...
  array[D] int<lower=0> which_dim_cor_reg;
  ...
  array[Lzeta   ] int<lower=0> zeta_sd_ind  ; // sd index for each column of z
  array[Lzeta_2 ] int<lower=0> zeta_sd_ind_2;
  array[Lzeta_3 ] int<lower=0> zeta_sd_ind_3 ;
  array[Lzeta_4 ] int<lower=0> zeta_sd_ind_4 ;
  array[Lzeta_5 ] int<lower=0> zeta_sd_ind_5 ;
  array[Lzeta_6 ] int<lower=0> zeta_sd_ind_6 ;
  array[Lzeta_7 ] int<lower=0> zeta_sd_ind_7 ;
  array[Lzeta_8 ] int<lower=0> zeta_sd_ind_8 ;
  array[Lzeta_9 ] int<lower=0> zeta_sd_ind_9 ;
  array[Lzeta_10 ] int<lower=0> zeta_sd_ind_10;
  array[Lzeta_11 ] int<lower=0> zeta_sd_ind_11;
  array[Lzeta_12 ] int<lower=0> zeta_sd_ind_12;
  array[Lzeta_13 ] int<lower=0> zeta_sd_ind_13;
  array[Lzeta_14 ] int<lower=0> zeta_sd_ind_14;
  array[Lzeta_15 ] int<lower=0> zeta_sd_ind_15;
  array[Lzeta_16 ] int<lower=0> zeta_sd_ind_16;
  array[Lzeta_17 ] int<lower=0> zeta_sd_ind_17;
  array[Lzeta_18 ] int<lower=0> zeta_sd_ind_18;
  array[Lzeta_19 ] int<lower=0> zeta_sd_ind_19;
  array[Lzeta_20 ] int<lower=0> zeta_sd_ind_20;
  array[Lzeta_21 ] int<lower=0> zeta_sd_ind_21;
  array[Lzeta_22 ] int<lower=0> zeta_sd_ind_22;
  array[Lzeta_23 ] int<lower=0> zeta_sd_ind_23;
  array[Lzeta_24 ] int<lower=0> zeta_sd_ind_24;
  array[Lzeta_25 ] int<lower=0> zeta_sd_ind_25;
  array[Lzeta_26 ] int<lower=0> zeta_sd_ind_26;
  array[Lzeta_27 ] int<lower=0> zeta_sd_ind_27;
  array[Lzeta_28 ] int<lower=0> zeta_sd_ind_28;
  array[Lzeta_29 ] int<lower=0> zeta_sd_ind_29;
  array[Lzeta_30 ] int<lower=0> zeta_sd_ind_30;
  array[Lzeta_31 ] int<lower=0> zeta_sd_ind_31;
  array[Lzeta_32 ] int<lower=0> zeta_sd_ind_32;
  ...
  array[Lzeta_cor] int<lower=0> cor_z_item_ind; // item index for each column of z for correlated random effs.
  array[Lzeta_cor] int<lower=0> cor_z_item_elem_ind; // element within item index for each column of z for correlated random effs.
  array[Lzeta_cor_2] int<lower=0> cor_z_item_ind_2;
  array[Lzeta_cor_2] int<lower=0> cor_z_item_elem_ind_2;
  array[Lzeta_cor_3] int<lower=0> cor_z_item_ind_3;
  array[Lzeta_cor_3] int<lower=0> cor_z_item_elem_ind_3;

  int<lower=0> Laeta_sd;
  array[Laeta] int<lower=0> alindex;
  array[Laeta] int<lower=0> aeta_sd_ind;
  array[Laeta_cor] int<lower=0> cor_a_item_ind;
  array[Laeta_cor] int<lower=0> cor_a_item_elem_ind;

  int<lower=0> Ldeta_sd;
  array[Ldeta] int<lower=0> dlindex;
  array[Ldeta] int<lower=0> deta_sd_ind;
  array[Ldeta_cor] int<lower=0> cor_d_item_ind;
  array[Ldeta_cor] int<lower=0> cor_d_item_elem_ind;
...
parameters {
  ...
  vector<lower=0>[nAlpha_r] alpha_r_l;      // structural regression, alpha
  ...
  array[l_Lzeta_cor] vector[u_Lzeta_cor] zeta_c;          // random regression pars

  // extra slots in the case of multiple separate regressions
  array[l_Lzeta_cor_2] vector[u_Lzeta_cor_2] zeta_c_2;
  array[l_Lzeta_cor_3] vector[u_Lzeta_cor_3] zeta_c_3;

  array[l_Laeta_cor] vector[u_Laeta_cor] aeta_c;
  array[l_Ldeta_cor] vector[u_Ldeta_cor] deta_c;
...
transformed parameters {
...

  {
    for(j in 1:J) {
      for(d in (j+1):D) {
        alpha[d, j] = 0.0; //negative_infinity();
      }
    }
    int index = 0;
    for(d in 1:D) {
...

  {
   ...
   // so I replaced it with:
      ...
      nu[i] = dot_product(theta[nn[i], ] + xb[i, ], col(alpha, jj[i]));
...
model {
  ...
  // if(L) {
    ...
    // recent
  // alpha_l ~ normal(-1.5, 1.0); // should these priors be wider?   -0.5, 1.0
  // if(LMean) {
  //   alpha_r_l ~ normal(-1.5, 1.0); // -0.5, 1.0
  // }
  alpha_l ~ lognormal(0.5, 1.0); // should these priors be wider?   -0.5, 1.0
  if(LMean) {
    alpha_r_l ~ lognormal(0.5, 1.0); // -0.5, 1.0
  }
  ...
  target += ordered_logistic_log_irt_vec(y, nu, c, eta3pl, Ncategi_jj, N_long, itype, fweights);
}
