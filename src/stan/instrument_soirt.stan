...
data {
  ...
  array[N_long] int<lower=1,upper=D> lambda_ind; // which 1st order dim does each obs. belong to?
  int<lower=0> L;        // number of non-zero loadings
  int<lower=0,upper=1> has_treg;  // do theta regression?
  array[has_treg ? 1 : 0] int<lower=1> beta_dstart; // beta start index for each dimension
  array[has_treg ? 1 : 0] int<lower=1> beta_dend;   // beta end index for each dimension
  array[any_rand_ind ? 1 : 0] int<lower=1> zeta_dstart; // zeta start index for each dimension
  array[any_rand_ind ? 1 : 0] int<lower=1> zeta_dend;   // zeta end index for each dimension
...
parameters {
  ...
  vector<lower=0.01>[D] sig_thetag_reg;          // residual uncertainty in thetag regression
  ...
  array[u_Lzeta_cor] vector[l_Lzeta_cor] zeta_c;          // random regression pars
  array[u_Laeta_cor] vector[l_Laeta_cor] aeta_c;
  array[u_Ldeta_cor] vector[l_Ldeta_cor] deta_c;
}
...

  {
    for(i in 1:N_long) {
      ...
      if(L) {
        ...
        nu[i] = dot_product(rep_vector(lambda_identify[lambda_ind[i]]*theta[nn[i], 1] + theta_resid[nn[i], lambda_ind[i]] + xb[i], D), col(alpha, jj[i]));
...
model {
  ...
  if(L) {
    alpha_l ~ lognormal(0.0, 0.5); // should these priors be wider?
    if(LMean) {
      alpha_r_l ~ lognormal(0.0, 0.5);
    }
  }
